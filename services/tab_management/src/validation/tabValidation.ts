...
const createTabValidation = (details: ITabInput) => {
  const { userId, venueId, paymentMethodId, status, preAuthAmountPesewas } =
    details;
  ...
  if (preAuthAmountPesewas === undefined || preAuthAmountPesewas === null) {
    errors.preAuthAmountPesewas = "Pre-auth amount is required";
  } else if (preAuthAmountPesewas <= 0) {
    errors.preAuthAmountPesewas = "Pre-auth amount must be greater than 0";
  }
...
const updateTabValidation = (details: Partial<ITabInput>) => {
  ...
  if (preAuthAmountPesewas !== undefined && preAuthAmountPesewas <= 0) {
    errors.preAuthAmountPesewas = "Pre-auth amount must be greater than 0";
  }
