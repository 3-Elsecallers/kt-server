...
const createTabItemValidation = (details: ITabItemInput) => {
  const { tabId, posItemId, itemName, quantity, unitPricePesewas } = details;
  ...
  if (unitPricePesewas === undefined || unitPricePesewas <= 0) {
    errors.unitPricePesewas = "Unit price must be greater than 0";
  }
