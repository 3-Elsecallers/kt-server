 ...
 * Get All Tabs From Specific Venue
 */
export const getUserTab = async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;

    const tabs = await prisma.tab.findUnique({
      where: {
  ...
  try {
    ...
    const updatedTab = await prisma.tab.update({
      ...
      data: {
        ...
        preAuthAmount:
          preAuthAmount ?? existingTab.preAuthAmount,
...
export const closeTab = async (req: Request, res: Response) => {
  try {
    const { tabId } = req.params;
    const { vatAmount, totalPaid } = req.body;
